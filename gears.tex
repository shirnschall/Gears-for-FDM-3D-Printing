...
%		\end{tikzpicture}
	...
	Start by drawing the outline of the gear. As we will see in \cref{non-circular} this can be any convex shape. As an example we will take a look at a circular gear. Then, using a compass, mark points with distance $r$ on the outline of your shape and draw a circle with radius $r$ on every second mark. We will see in \cref{section-gears} or in the examples how to calculate $r$.
	\begin{figure}[h!]
...
%		\end{align*}
	...
	\begin{mytheorem}\label{theorem-circle}
		Given a circle $K$ with radius $R$, $(R,n\in\mathbb{N}^*,m_0\in K,M)$ is a gear. Let $\varphi = \frac{Pi}{2n}$ then
		\begin{align}
	...
	\begin{mytheorem}\label{therem-rectangle}
		...
		Let $K$ be a rectangle with side lengths $a$ and $b$, center point M and corners $A,B,C$ and $D$. Let \begin{align}
			r\coloneqq\frac{a+b}{n},
	...
	\begin{mytheorem}\label{theorem:non-circular}
		Let $\gamma$ be a simple, closed curve.\\
